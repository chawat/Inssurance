...
const Cargoadmin = () => {
  ...
  const fetchQuotes = async () => {
    try {
      const response = await axios.get('http://localhost:3003/api/getPendingCargoData');
      console.log('Response data:', response.data);
      setQuotes(response.data.cargoData); // Update the state directly with response data
    } catch (error) {
