...
const Grouppersonaladmin = () => {
  const [quotes, setQuotes] = useState([]);

  const fetchQuotes = async () => {
    try {
      const response = await axios.get('http://localhost:3003/api/getPendingGroupPersonalData');
      console.log('Response data:', response.data);
      setQuotes(response.data.groupPersonalData); // Update the state directly with response data
    } catch (error) {
        ...
        <div className="contenthouse">
          {quotes.length > 0 && (
            <div className="quote-boxhouse">
              <h2>Group Personal Insurance</h2>
...
export default Grouppersonaladmin;
